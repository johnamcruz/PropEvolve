...
[R2D2](https://openreview.net/pdf?id=r1lyTjAqYX) illustrates the interaction between recurrent state and replay: it stores fixed-length sequences, never crosses episode boundaries, and unrolls recurrent networks over the replayed sequence. [Neural Episodic Control](https://arxiv.org/abs/1703.01988) is a distinct external-memory design that stores state representations and rapidly updated value estimates for context-based lookup. PropEvolve therefore starts with an explicitly off-policy recurrent value learner whose assumptions match sequence replay, rather than an on-policy optimizer.

## PropEvolve environment contract

PropEvolve's first experiment needs a grounded environment with these native interfaces:

- The environment advances through causal market decisions and exposes account, position, execution, and challenge-progress observations.
- Account observations include realized P&L, equity, peak equity, drawdown, distance to the maximum-loss-limit floor, and remaining episode time.
- Verified feedback includes realized reward, costs, drawdown pressure, pass, timeout, and maximum-loss-limit outcomes.
- The market observation uses an authenticated frozen FFM representation rather than an inherited trading signal.

PropEvolve's observation is explicit:
...
For the initial action space, exact enumeration remains better than AlphaCode-style massive sampling. At a decision point, the valid set is intentionally small: abstain, enter one contract long/short while flat, or hold/close while positioned. The value network estimates each action's account-conditioned return distribution. A deterministic risk layer must reject actions that violate prop constraints. Pyramiding and conviction sizing remain later, separately validated revisions. Diversity and clustering become useful in the slower outer loop, where several materially different policy candidates may otherwise duplicate the same behavior. Tree search becomes justified only if we later have a validated short-horizon market/fill model; otherwise imagined future prices are an unreliable verifier.
...
For the first POC, store these records but do not retrieve them directly into live inference. Train a recurrent distributional Double-DQN from bounded, balanced, episode-safe sequence replay and test whether recurrent state adds value over a matched feed-forward distributional Double-DQN baseline. The C51 value distribution represents economically different return outcomes for every valid discrete action, while Double-DQN reduces maximization bias. Historical and shadow episodes may enter replay only through authenticated causal trajectories; they never alter the live champion intraday.
...
- recurrent distributional Double-DQN is the first temporal-memory test, while direct external episodic retrieval remains a separate later ablation;
...
2. Use PropEvolve's causal prop-challenge environment and account/MLL state, with the frozen FFM embedding as market state.
3. Compare feed-forward and recurrent distributional Double-DQN using the same actions, reward, replay population, seeds, and compute.
...
7. Only after recurrent sequence learning shows matched temporal lift, test prioritized replay, alternative distributional value parameterizations, or external episodic retrieval—one mechanism at a time.
