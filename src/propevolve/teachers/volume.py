"""Portable Volume-participation teacher over frozen Chronos2 trajectories.

Volume is distilled as training-only context.  It never selects an action and
is not loaded by the deployed reasoning policy.
"""

from __future__ import annotations

from dataclasses import dataclass
import hashlib
import json
from pathlib import Path
import tempfile
from typing import ClassVar, Sequence

import numpy as np
import pandas as pd
import torch
from torch import nn
import torch.nn.functional as F

from ..cache import EmbeddingCache
from ..hardware import resolve_device
from .base import BaseTeacher
from .expansion import _flush_scores, _validate_source_receipt


TEACHER_CACHE_SCHEMA = "propevolve_volume_teacher_cache_v1"
ARTIFACT_SCHEMA = "volume-factorized-artifact-v2"
TRAINING_SCHEMA = "volume-factorized-training-v2"
MODEL_SCHEMA = "frozen-chronos2-factorized-volume-participation-transformer-v1"
CONTEXT_LENGTH = 50
PRECURSOR_HORIZONS = (5, 10, 20, 50)
CHANNELS = (
    "long_participation_probability",
    "long_clean_given_participation_probability",
    "short_participation_probability",
    "short_clean_given_participation_probability",
)


def _sha256(path: Path) -> str:
    digest = hashlib.sha256()
    with Path(path).open("rb") as stream:
        for block in iter(lambda: stream.read(1 << 20), b""):
            digest.update(block)
    return digest.hexdigest()


def _canonical_sha256(payload: object) -> str:
    encoded = json.dumps(
        payload, allow_nan=False, separators=(",", ":"), sort_keys=True
    ).encode()
    return hashlib.sha256(encoded).hexdigest()


def _utc_boundary(value: str) -> np.datetime64:
    parsed = pd.Timestamp(value)
    parsed = (
        parsed.tz_localize("UTC")
        if parsed.tzinfo is None
        else parsed.tz_convert("UTC")
    )
    return np.datetime64(parsed.tz_localize(None), "ns")


@dataclass(frozen=True)
class _VolumeConfig:
    embedding_dim: int
    model_dim: int
    num_heads: int
    feedforward_dim: int
    num_layers: int
    dropout: float
    context_length: int
    precursor_horizons: tuple[int, ...]

    @classmethod
    def from_payload(cls, payload: dict) -> "_VolumeConfig":
        values = dict(payload)
        values["precursor_horizons"] = tuple(values["precursor_horizons"])
        config = cls(**values)
        if (
            config.context_length != CONTEXT_LENGTH
            or config.precursor_horizons != PRECURSOR_HORIZONS
            or min(
                config.embedding_dim,
                config.model_dim,
                config.num_heads,
                config.feedforward_dim,
                config.num_layers,
            ) < 1
            or config.model_dim % config.num_heads
            or not 0.0 <= float(config.dropout) < 1.0
        ):
            raise ValueError("Volume model configuration is unsupported")
        return config


class _FrozenInputLayerNorm(nn.LayerNorm):
    def forward(self, values: torch.Tensor) -> torch.Tensor:
        normalized = F.layer_norm(
            values, self.normalized_shape, weight=None, bias=None, eps=self.eps
        )
        return normalized * self.weight + self.bias


class _DirectionalVolumeExpert(nn.Module):
    def __init__(self, config: _VolumeConfig):
        super().__init__()
        self.config = config
        self.input_norm = _FrozenInputLayerNorm(config.embedding_dim)
        self.input_projection = nn.Linear(config.embedding_dim, config.model_dim)
        self.age_embedding = nn.Embedding(config.context_length, config.model_dim)
        self.horizon_embedding = nn.Embedding(
            len(config.precursor_horizons), config.model_dim
        )
        layer = nn.TransformerEncoderLayer(
            d_model=config.model_dim,
            nhead=config.num_heads,
            dim_feedforward=config.feedforward_dim,
            dropout=float(config.dropout),
            activation="gelu",
            batch_first=True,
            norm_first=True,
        )
        self.temporal_encoder = nn.TransformerEncoder(
            layer, num_layers=config.num_layers, enable_nested_tensor=False
        )
        self.view_projection = nn.Sequential(
            nn.Linear(config.model_dim * 2, config.model_dim),
            nn.GELU(),
            nn.LayerNorm(config.model_dim),
        )
        self.fusion = nn.Sequential(
            nn.Linear(
                config.model_dim * len(config.precursor_horizons),
                config.model_dim,
            ),
            nn.GELU(),
            nn.LayerNorm(config.model_dim),
        )
        self.participation_head = nn.Linear(config.model_dim, 1)
        self.quality_head = nn.Linear(config.model_dim, 1)
        for index, horizon in enumerate(config.precursor_horizons):
            positions = torch.arange(horizon, dtype=torch.long)[None, :]
            self.register_buffer(
                f"_age_index_{index}", horizon - 1 - positions, persistent=False
            )
            self.register_buffer(
                f"_view_index_{index}",
                torch.full_like(positions, index),
                persistent=False,
            )
            self.register_buffer(
                f"_causal_mask_{index}",
                torch.triu(torch.ones((horizon, horizon), dtype=torch.bool), diagonal=1),
                persistent=False,
            )

    def forward(self, trajectory: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        projected = self.input_projection(self.input_norm(trajectory))
        summaries = []
        for index, horizon in enumerate(self.config.precursor_horizons):
            view = projected[:, -horizon:]
            view = (
                view
                + self.age_embedding(getattr(self, f"_age_index_{index}"))
                + self.horizon_embedding(getattr(self, f"_view_index_{index}"))
            )
            encoded = self.temporal_encoder(
                view,
                mask=getattr(self, f"_causal_mask_{index}"),
                is_causal=True,
            )
            summaries.append(
                self.view_projection(
                    torch.cat((encoded[:, -1], encoded.mean(dim=1)), dim=-1)
                )
            )
        fused = self.fusion(torch.cat(summaries, dim=-1))
        return (
            self.participation_head(fused).squeeze(-1),
            self.quality_head(fused).squeeze(-1),
        )


class _VolumeModel(nn.Module):
    def __init__(self, config: _VolumeConfig):
        super().__init__()
        self.long_expert = _DirectionalVolumeExpert(config)
        self.short_expert = _DirectionalVolumeExpert(config)

    def forward(self, trajectory: torch.Tensor) -> torch.Tensor:
        long_participation, long_quality = self.long_expert(trajectory)
        short_participation, short_quality = self.short_expert(trajectory)
        return torch.stack(
            (
                long_participation,
                long_quality,
                short_participation,
                short_quality,
            ),
            dim=-1,
        )


@dataclass(frozen=True)
class VolumeTeacher(BaseTeacher):
    """Authenticated Volume artifact producing calibrated factor probabilities."""

    model: _VolumeModel
    stream_names: tuple[str, ...]
    calibration_scales: torch.Tensor
    calibration_biases: torch.Tensor

    kind: ClassVar[str] = "volume"
    channels: ClassVar[tuple[str, ...]] = CHANNELS

    @classmethod
    def load(cls, manifest_path: str | Path, *, device: str) -> "VolumeTeacher":
        manifest_path = Path(manifest_path).resolve(strict=True)
        manifest = json.loads(manifest_path.read_text())
        volume = manifest.get("volume")
        if (
            manifest.get("schema") != "propevolve_verified_teacher_assets_v1"
            or manifest.get("status") != "verified_checkpoint_only"
            or manifest.get("inference_dependency") is not False
            or not isinstance(volume, dict)
        ):
            raise ValueError("verified Volume teacher manifest is invalid")
        checkpoint = manifest_path.parent / str(volume.get("checkpoint", ""))
        checkpoint_sha256 = _sha256(checkpoint)
        if checkpoint_sha256 != volume.get("checkpoint_sha256"):
            raise ValueError("Volume teacher checkpoint identity drifted")
        payload = torch.load(checkpoint, map_location="cpu", weights_only=True)
        if (
            payload.get("schema") != ARTIFACT_SCHEMA
            or payload.get("schema") != volume.get("artifact_schema")
            or payload.get("training_schema") != TRAINING_SCHEMA
            or payload.get("training_schema") != volume.get("training_schema")
            or payload.get("model_metadata", {}).get("schema") != MODEL_SCHEMA
            or payload.get("model_metadata", {}).get("schema")
            != volume.get("model_schema")
            or not isinstance(payload.get("state_dict"), dict)
            or not isinstance(payload.get("calibrator"), dict)
        ):
            raise ValueError("Volume teacher artifact contract drifted")
        lineage = payload.get("lineage") or {}
        if (
            lineage.get("encoder_sha256")
            != volume.get("encoder_checkpoint_sha256")
            or lineage.get("encoder_identity_sha256")
            != volume.get("encoder_identity_sha256")
            or lineage.get("dataset_lineage_sha256")
            != volume.get("dataset_lineage_sha256")
            or set(lineage.get("cache_sha256s", ()))
            != set((volume.get("cache_sha256s") or {}).values())
        ):
            raise ValueError("Volume teacher lineage does not match its caches")
        calibration = payload["calibrator"]
        parameters = dict(calibration)
        reported_sha256 = parameters.pop("parameters_sha256", None)
        if (
            reported_sha256 != payload.get("calibrator_sha256")
            or _canonical_sha256(parameters) != reported_sha256
            or calibration.get("side_order") != ["long", "short"]
            or calibration.get("factor_order")
            != ["participation", "quality_given_participation"]
        ):
            raise ValueError("Volume teacher calibration identity drifted")
        streams = calibration.get("streams")
        if not isinstance(streams, list) or not streams:
            raise ValueError("Volume teacher calibration lacks streams")
        names, scales, biases = [], [], []
        for stream_id, stream in enumerate(streams):
            if stream.get("stream_id") != stream_id:
                raise ValueError("Volume teacher stream ordering drifted")
            names.append(str(stream["stream_name"]))
            stream_scales, stream_biases = [], []
            for side in ("long", "short"):
                for factor in ("participation", "quality_given_participation"):
                    values = stream["sides"][side][factor]
                    stream_scales.append(float(values["scale"]))
                    stream_biases.append(float(values["bias"]))
            scales.append(stream_scales)
            biases.append(stream_biases)
        if set(names) != {f"{ticker}@3min" for ticker in volume["universe"]}:
            raise ValueError("Volume teacher calibrated universe drifted")
        config = _VolumeConfig.from_payload(payload["model_config"])
        model = _VolumeModel(config)
        model.load_state_dict(payload["state_dict"], strict=True)
        resolved_device = resolve_device(device)
        model.to(resolved_device).eval()
        return cls(
            model=model,
            device=resolved_device,
            stream_names=tuple(names),
            calibration_scales=torch.as_tensor(
                scales, dtype=torch.float32, device=resolved_device
            ),
            calibration_biases=torch.as_tensor(
                biases, dtype=torch.float32, device=resolved_device
            ),
            manifest=manifest,
            checkpoint_sha256=checkpoint_sha256,
            context_length=config.context_length,
            embedding_dim=config.embedding_dim,
        )

    def _score_trusted(
        self, trajectory: torch.Tensor, *, ticker: str | None = None
    ) -> torch.Tensor:
        stream = f"{ticker}@3min"
        if ticker is None or stream not in self.stream_names:
            raise ValueError("Volume teacher requires a calibrated ticker")
        stream_id = self.stream_names.index(stream)
        logits = self.model(trajectory.to(self.device, dtype=torch.float32))
        return torch.sigmoid(
            logits * self.calibration_scales[stream_id]
            + self.calibration_biases[stream_id]
        )


@dataclass(frozen=True)
class VolumeTeacherCache:
    root: Path
    probabilities: np.ndarray
    availability: np.ndarray
    timestamps: np.ndarray
    manifest: dict

    @classmethod
    def load(cls, root: str | Path) -> "VolumeTeacherCache":
        root = Path(root).resolve(strict=True)
        manifest = json.loads((root / "manifest.json").read_text())
        if (
            manifest.get("schema") != TEACHER_CACHE_SCHEMA
            or tuple(manifest.get("channels", ())) != CHANNELS
            or manifest.get("selection_rows_included") is not False
            or manifest.get("sealed_rows_included") is not False
        ):
            raise ValueError("Volume teacher cache contract is invalid")
        paths = {
            name: root / f"{name}.npy"
            for name in ("probabilities", "availability", "timestamps")
        }
        for name, path in paths.items():
            if _sha256(path) != manifest.get(f"{name}_sha256"):
                raise ValueError(f"Volume teacher cache {name} identity drifted")
        probabilities = np.load(paths["probabilities"], mmap_mode="r")
        availability = np.load(paths["availability"], mmap_mode="r")
        timestamps = np.load(paths["timestamps"], mmap_mode="r")
        rows = int(manifest.get("rows", -1))
        available = (
            probabilities[availability]
            if availability.shape == (rows,) and availability.dtype == np.bool_
            else np.empty((0, len(CHANNELS)), dtype=np.float32)
        )
        if (
            probabilities.shape != (rows, len(CHANNELS))
            or probabilities.dtype != np.float32
            or availability.shape != (rows,)
            or availability.dtype != np.bool_
            or timestamps.shape != (rows,)
            or np.isnat(timestamps).any()
            or (len(timestamps) > 1 and not np.all(timestamps[1:] > timestamps[:-1]))
            or np.any(timestamps >= _utc_boundary(manifest["training_end_exclusive"]))
            or not np.isfinite(available).all()
            or np.any(available < 0.0)
            or np.any(available > 1.0)
        ):
            raise ValueError("Volume teacher cache arrays are invalid")
        return cls(root, probabilities, availability, timestamps, manifest)


@dataclass(frozen=True)
class VolumeTeacherTargets:
    probabilities: dict[str, np.ndarray]
    availability: dict[str, np.ndarray]

    @classmethod
    def load(
        cls, cache_root: str | Path, markets: dict[str, object]
    ) -> "VolumeTeacherTargets":
        root = Path(cache_root)
        probabilities, availability = {}, {}
        for ticker, market in markets.items():
            cache = VolumeTeacherCache.load(root / ticker)
            market_timestamps = np.asarray(getattr(market, "timestamps"))
            indices = np.searchsorted(cache.timestamps, market_timestamps)
            if (
                (indices >= len(cache.timestamps)).any()
                or not np.array_equal(cache.timestamps[indices], market_timestamps)
            ):
                raise ValueError(
                    f"Volume teacher rows do not align to training market {ticker}"
                )
            probabilities[ticker] = cache.probabilities[indices]
            availability[ticker] = cache.availability[indices]
        return cls(probabilities, availability)

    def target(self, ticker: str, row: int) -> np.ndarray | None:
        if ticker not in self.probabilities:
            raise ValueError(f"Volume teacher has no aligned market {ticker}")
        if row < 0 or row >= len(self.probabilities[ticker]):
            raise IndexError("Volume teacher row is outside the aligned market")
        if not bool(self.availability[ticker][row]):
            return None
        return np.asarray(self.probabilities[ticker][row], dtype=np.float32)


def _load_embedding_source(
    *,
    teacher: VolumeTeacher,
    embedding_cache: str | Path,
    ticker: str,
    expected_cache_identity_sha256: str,
) -> EmbeddingCache:
    source = EmbeddingCache.load(embedding_cache)
    volume = teacher.manifest["volume"]
    if (
        source.manifest.get("ticker") != ticker
        or source.manifest.get("encoder_identity_sha256")
        != volume["encoder_identity_sha256"]
        or source.manifest.get("imported_ffm_cache_identity_sha256")
        != expected_cache_identity_sha256
        or volume["cache_identity_sha256s"].get(ticker)
        != expected_cache_identity_sha256
    ):
        raise ValueError("Volume teacher cache identity mismatch")
    _validate_source_receipt(
        Path(embedding_cache),
        source.manifest,
        expected_cache_identity_sha256=expected_cache_identity_sha256,
        expected_embeddings_sha256=volume["embeddings_sha256s"][ticker],
    )
    if source.embeddings.shape[1] != teacher.embedding_dim:
        raise ValueError("Volume teacher embedding width mismatch")
    return source


def build_volume_teacher_cache(
    *,
    teacher: VolumeTeacher,
    embedding_cache: str | Path,
    destination: str | Path,
    ticker: str,
    training_end_exclusive: str,
    expected_cache_identity_sha256: str,
    batch_size: int,
    synchronization_batches: int,
    progress_batches: int = 100,
) -> Path:
    if min(batch_size, synchronization_batches, progress_batches) < 1:
        raise ValueError("Volume teacher batch settings must be positive")
    source = _load_embedding_source(
        teacher=teacher,
        embedding_cache=embedding_cache,
        ticker=ticker,
        expected_cache_identity_sha256=expected_cache_identity_sha256,
    )
    volume = teacher.manifest["volume"]
    boundary = _utc_boundary(training_end_exclusive)
    if boundary >= _utc_boundary("2026-01-01"):
        raise ValueError("Volume teacher training boundary crosses sealed data")
    first = int(np.searchsorted(source.timestamps, _utc_boundary("2021-01-01")))
    stop = int(np.searchsorted(source.timestamps, boundary))
    if stop <= first or stop < teacher.context_length:
        raise ValueError("Volume teacher cache has insufficient training history")
    timestamps = source.timestamps[first:stop]
    rows = len(timestamps)
    expected_contract = {
        "schema": TEACHER_CACHE_SCHEMA,
        "channels": list(CHANNELS),
        "rows": rows,
        "ticker": ticker,
        "training_start_inclusive": "2021-01-01",
        "training_end_exclusive": training_end_exclusive,
        "selection_rows_included": False,
        "sealed_rows_included": False,
        "checkpoint_sha256": teacher.checkpoint_sha256,
        "embedding_cache_identity_sha256": expected_cache_identity_sha256,
        "encoder_identity_sha256": volume["encoder_identity_sha256"],
        "context_length": teacher.context_length,
        "embedding_dim": teacher.embedding_dim,
    }
    destination = Path(destination)
    if destination.exists():
        existing = VolumeTeacherCache.load(destination)
        if any(
            existing.manifest.get(key) != value
            for key, value in expected_contract.items()
        ):
            raise ValueError("existing Volume teacher cache identity conflicts")
        return destination.resolve()
    destination.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(
        prefix=f".{destination.name}.", dir=destination.parent
    ) as temporary_value:
        temporary = Path(temporary_value)
        probabilities = np.lib.format.open_memmap(
            temporary / "probabilities.npy",
            mode="w+",
            dtype=np.float32,
            shape=(rows, len(CHANNELS)),
        )
        probabilities[:] = 0.0
        availability = np.lib.format.open_memmap(
            temporary / "availability.npy",
            mode="w+",
            dtype=np.bool_,
            shape=(rows,),
        )
        availability[:] = False
        np.save(temporary / "timestamps.npy", timestamps)
        first_endpoint = max(first, teacher.context_length - 1)
        windows = np.lib.stride_tricks.sliding_window_view(
            source.embeddings[:stop],
            window_shape=teacher.context_length,
            axis=0,
        ).transpose(0, 2, 1)
        window_start = first_endpoint - (teacher.context_length - 1)
        window_stop = stop - (teacher.context_length - 1)
        output_start = first_endpoint - first
        output_cursor = output_start
        pending: list[torch.Tensor] = []
        total_batches = (window_stop - window_start + batch_size - 1) // batch_size
        with torch.inference_mode():
            for batch_number, batch_start in enumerate(
                range(window_start, window_stop, batch_size), start=1
            ):
                batch_stop = min(batch_start + batch_size, window_stop)
                batch = np.ascontiguousarray(windows[batch_start:batch_stop])
                pending.append(
                    teacher._score_trusted(torch.from_numpy(batch), ticker=ticker)
                )
                if (
                    len(pending) >= synchronization_batches
                    or batch_number == total_batches
                ):
                    output_cursor = _flush_scores(
                        pending, probabilities, destination_start=output_cursor
                    )
                if batch_number % progress_batches == 0:
                    print(
                        f"[volume-teacher] {ticker} "
                        f"batch={batch_number}/{total_batches}",
                        flush=True,
                    )
        if output_cursor != rows:
            raise RuntimeError("Volume teacher output row accounting drifted")
        availability[output_start:] = True
        probabilities.flush()
        availability.flush()
        del probabilities, availability
        manifest = {
            **expected_contract,
            **{
                f"{name}_sha256": _sha256(temporary / f"{name}.npy")
                for name in ("probabilities", "availability", "timestamps")
            },
            "available_rows": rows - output_start,
            "device": str(teacher.device),
            "batch_size": int(batch_size),
            "synchronization_batches": int(synchronization_batches),
        }
        (temporary / "manifest.json").write_text(
            json.dumps(manifest, indent=2, sort_keys=True) + "\n"
        )
        VolumeTeacherCache.load(temporary)
        temporary.rename(destination)
    return destination.resolve()


def load_builder_config(path: str | Path) -> dict:
    path = Path(path).resolve(strict=True)
    payload = json.loads(path.read_text())
    required = {
        "schema",
        "teacher_manifest",
        "embedding_cache_root",
        "output_root",
        "tickers",
        "training_start_inclusive",
        "training_end_exclusive",
        "sealed_start",
        "device",
        "batch_size",
        "fallback_batch_sizes",
        "synchronization_batches",
        "progress_batches",
    }
    if (
        set(payload) != required
        or payload.get("schema") != "volume_teacher_cache_build_v1"
    ):
        raise ValueError("Volume teacher builder configuration is invalid")
    tickers = tuple(str(value) for value in payload["tickers"])
    batches = (
        int(payload["batch_size"]),
        *(int(value) for value in payload["fallback_batch_sizes"]),
    )
    if (
        not tickers
        or len(set(tickers)) != len(tickers)
        or any(value < 1 for value in batches)
        or len(set(batches)) != len(batches)
        or not _utc_boundary(payload["training_start_inclusive"])
        < _utc_boundary(payload["training_end_exclusive"])
        < _utc_boundary(payload["sealed_start"])
    ):
        raise ValueError("Volume teacher builder ranges are invalid")
    payload["tickers"] = tickers
    payload["batch_sizes"] = batches
    payload["_root"] = str(path.parent.parent)
    return payload


def build_volume_teacher_caches(
    config_path: str | Path, *, requested_tickers: Sequence[str] = ()
) -> tuple[Path, ...]:
    config = load_builder_config(config_path)
    root = Path(config["_root"])
    tickers = tuple(requested_tickers) or config["tickers"]
    if not set(tickers) <= set(config["tickers"]):
        raise ValueError("requested Volume teacher ticker is outside the contract")
    teacher = VolumeTeacher.load(
        root / config["teacher_manifest"], device=config["device"]
    )
    volume = teacher.manifest["volume"]
    results = []
    for ticker in tickers:
        destination = root / config["output_root"] / ticker
        last_error = None
        for batch_size in config["batch_sizes"]:
            try:
                results.append(
                    build_volume_teacher_cache(
                        teacher=teacher,
                        embedding_cache=root
                        / config["embedding_cache_root"]
                        / ticker,
                        destination=destination,
                        ticker=ticker,
                        training_end_exclusive=config["training_end_exclusive"],
                        expected_cache_identity_sha256=volume[
                            "cache_identity_sha256s"
                        ][ticker],
                        batch_size=batch_size,
                        synchronization_batches=int(
                            config["synchronization_batches"]
                        ),
                        progress_batches=int(config["progress_batches"]),
                    )
                )
                print(
                    f"[volume-teacher] COMPLETE {ticker} {results[-1]}",
                    flush=True,
                )
                break
            except RuntimeError as exc:
                if (
                    "out of memory" not in str(exc).lower()
                    or batch_size == config["batch_sizes"][-1]
                ):
                    raise
                last_error = exc
                if teacher.device.type == "mps":
                    torch.mps.empty_cache()
        else:  # pragma: no cover
            raise RuntimeError(
                "Volume teacher batch fallbacks exhausted"
            ) from last_error
    return tuple(results)


__all__ = [
    "CHANNELS",
    "VolumeTeacher",
    "VolumeTeacherCache",
    "VolumeTeacherTargets",
    "build_volume_teacher_cache",
    "build_volume_teacher_caches",
    "load_builder_config",
]
