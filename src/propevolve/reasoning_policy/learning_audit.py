...
def score_labeled_examples(policy, records):
    ...
    from .staged_preparation import record_context
    from .staged_metrics import assessment_advantages
    runtime = policy.policy if isinstance(policy, ReasoningPolicy) else policy
    for record in records:
        answer = record["messages"][-1]["content"]
        choices = legal_completion_names(answer)
        supplied = json.loads(record["messages"][-2]["content"])["legal_actions"]
        ...
        context = record_context(record, runtime.settings["staged_policy"]["state_fields"])
        result = runtime.assess(context, tuple(Action[name] for name in choices))
        ...
        row = {
            ...
            "scores": scores,
            "score_type": "log_probability", "assessment": result["assessment"],
...
def assess_prepared(config_path, view, *, role, output, root=None):
    ...
    if config.get("architecture") != "staged_reasoning_v1":
        raise ValueError("assessment requires staged_reasoning_v1 architecture")
    policy = StagedReasoningPolicy.from_config(config_path, root=root)
    groups = {}
    with (destination / "scores.jsonl").open("x") as stream:
        def record_score(index, scores):
            ...
            values = dict(zip(names, scores["log_probs"]))
            predicted = select_legal_action(scores["assessment"], [Action[name] for name in names]).name
            trace = {"score_type": "log_probability",
                "assessment": dict(zip(("entry", "direction", "management"), scores["assessment"])),
                "interpretation": dict(zip(prepared_row["staged_queries"]["channel_names"],
            ...
            row = {"index": index, "source_id": original["source_id"],
                ...
                "specialist_targets": original["targets"].get("specialist_targets", {}), **trace}
            from .staged_metrics import assessment_advantages
            row["joint_log_probability_advantage"] = row["target_advantage"]
            row["target_advantage"] = min(assessment_advantages(row).values())
            ...
            grouped[1].append(scores["assessment"])
        ...
        metrics = evaluate_staged_validation(policy.backend, dataset, config, on_scored=record_score)
    from .staged_metrics import boundary_metrics as metric
...
def main(argv=None):
    ...
    parser.add_argument("--config", required=True, help="JSON staged model settings")
    ...
    policy = StagedReasoningPolicy.from_config(args.config)
