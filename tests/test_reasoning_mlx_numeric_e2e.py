...
def test_error_selected_action_update_uses_action_and_teacher_targets_on_same_row():
    ...
    model = tiny_backbone()
    row = {**example(), "target_name": "ENTER_LONG_1", "error_selected_distillation": {
...
def test_frozen_action_validation_does_not_require_training_only_anchor_tensors():
    ...
    config = {
        "seed": 17, "validation_batch_size": 1, "max_seq_length": 8,
        "input_mode": "embeddings", "decision_objective": "hierarchical_binary",
        "action_supervision": {"enabled": True, "soft_target_weight": 1.,
                               "ranking_weight": 2., "margin": .25},
    ...
    result = evaluate_action_validation(model, [row], config)

    assert result["val_loss"] > 0


def test_frozen_action_validation_batch_two_matches_batch_one():
    from propevolve.reasoning_policy.supervised_trainer import evaluate_action_validation
    ...
    paired = evaluate_action_validation(model, rows, {
