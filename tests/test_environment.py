...
def test_reaching_breakeven_records_recovery_and_continues_the_challenge() -> None:
    market = _recovery_market(opens=(100.0, 100.0, 235.0, 235.0, 235.0, 235.0))
    env = HistoricalChallengeEnv(
        {"NQ": market},
        round_trip_fees={"NQ": 0.0},
        tick_values={"NQ": 20.0},
    ...
    _, _, terminated, _, recovered = env.step(Action.CLOSE)
...
def test_recovery_latch_keeps_native_entries_available_after_breakeven_giveback(
) -> None:
    market = _recovery_market(
        opens=(100.0, 100.0, 235.0, 235.0, 220.0, 220.0, 220.0),
    )
    env = HistoricalChallengeEnv(
        {"NQ": market},
        round_trip_fees={"NQ": 0.0},
    ...
    env.step(Action.ENTER_LONG_1)
    _, _, terminated, _, gave_back = env.step(Action.CLOSE)

    assert recovered["recovery_status"] == "recovered"
    assert not terminated
    assert gave_back["realized_pnl"] == -300.0
    assert gave_back["recovery_status"] == "recovered"
    assert gave_back["valid_actions"] == (
        Action.WAIT,
        Action.ENTER_LONG_1,
        Action.ENTER_SHORT_1,
    )

    _, next_challenge = env.reset(options={"ticker": "NQ", "start": 0})
    assert next_challenge["valid_actions"] == (Action.WAIT,)


def test_normal_challenge_latches_recovery_only_after_realized_pnl_turns_red(
) -> None:
    market = _recovery_market(
        opens=(100.0, 100.0, 85.0, 85.0, 100.0, 100.0, 100.0),
    )
    env = HistoricalChallengeEnv(
        {"NQ": market},
        round_trip_fees={"NQ": 0.0},
    ...
    _, normal = env.reset(options={
        "ticker": "NQ",
        "start": 0,
        "latch_recovery_until_terminal": True,
    })

    assert "recovery_status" not in normal
    env.step(Action.ENTER_LONG_1)
    _, _, terminated, _, red = env.step(Action.CLOSE)

    assert not terminated
    assert red["realized_pnl"] == -300.0
    assert "recovery_status" not in red
    assert red["valid_actions"] == (
        Action.WAIT,
        Action.ENTER_LONG_1,
        Action.ENTER_SHORT_1,
    )
